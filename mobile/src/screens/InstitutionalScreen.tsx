...
// Per-strategy colour + one-line description (from the reference books).
const STRAT_META: Record<string, { color: string; blurb: string }> = {
  momentum: { color: theme.green, blurb: 'Riding a strong, persistent price trend.' },
  trend: { color: theme.accent, blurb: 'Established up-trend — EMAs stacked / golden cross.' },
  breakout: { color: GOLD, blurb: 'A volatility squeeze resolving into a new high.' },
  mean_rev: { color: '#c77dff', blurb: 'Stretched far from its mean — contrarian bounce.' },
  stat_arb: { color: '#4dd0e1', blurb: 'Cheap vs the index — relative-value convergence.' },
};
...
function StrategyTag({ s, small }: { s: StrategyHit; small?: boolean }) {
    ...
    <TouchableOpacity style={styles.row} onPress={onOpen} activeOpacity={0.7}>
      ...
      <View style={styles.rowRight}>
        <View style={styles.scoreBox}>
          ...
          <Text style={styles.scoreLbl}>strategy score</Text>
  ...
  return (
    <Sheet onClose={onClose} maxHeight="92%">
        <ScrollView bounces={false}>
          ...
          {/* The heart of this screen: which strategies flagged the name, and why. */}
          <Text style={styles.secTitle}>STRATEGIES MATCHED</Text>
          ...
          <Text style={styles.disc}>
            Strategies drawn from The Algorithmic Trading Handbook and a review of algorithmic HFT strategies:
            momentum, trend-following, breakout, mean-reversion and statistical arbitrage. Indicative and
...
export default function InstitutionalScreen() {
  const [recs, setRecs] = useState<InstitutionalRec[]>(() => getCache()?.recs || []);
  ...
  const runScan = useCallback(async () => {
    ...
    try {
      ...
      const analysed: InstitutionalRec[] = [];
      ...
      const run = async (sym: string) => {
        try {
          const rec = await api.institutional(sym, names[sym.toUpperCase()]?.name);
          if (token.cancelled) return;
          if (rec && !rec.error) {
            analysed.push(rec);
          ...
          if (!token.cancelled) {
            ...
            setStatus(`Scanning ${done}/${total} · ${live.size} strategy match${live.size === 1 ? '' : 'es'}`);
          }
        }
      };
      let i = 0;
      ...
      if (token.cancelled) return;
      const now = Date.now();
      await mergeInst(analysed, d, now);
      const list = getCache()?.recs || [];
      setRecs(list);
      setAsof(now);
      setStatus(`${list.length} strategy match${list.length === 1 ? '' : 'es'} · scanned ${total} mid/large caps`);
    } catch (e) {
      if (!token.cancelled) setError(e instanceof Error ? e.message : 'Failed to build institutional list');
    } finally {
      if (!token.cancelled) {
  ...
  // Which strategies appear in the current list — drives the filter chips.
  const availStrategies = useMemo(() => {
  ...
  const onWatch = async (r: InstitutionalRec) => {
    setWatch(await addSymbol(watch, r.symbol));
    toast(`${r.symbol} added to watchlist`);
  };
  const onAlert = async (r: InstitutionalRec) => {
  ...
  const onChart = (r: InstitutionalRec) => {
    setOpen(null);
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.body}>
        {!scanning && error ? <EmptyState icon="⚠" title="Couldn't build institutional list" hint={error} /> : null}
...
const styles = StyleSheet.create({
  ...
  sheet: { maxHeight: '92%', backgroundColor: theme.surface, borderTopLeftRadius: 16, borderTopRightRadius: 16, borderColor: theme.border2, borderWidth: 1, padding: theme.sp.lg, alignSelf: 'center', width: '100%', maxWidth: 620 },
