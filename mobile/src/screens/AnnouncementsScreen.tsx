...
// `embedded` renders the list inline inside another page's scroller: no page
// chrome, no second scroll view, and no duplicate heading — the host supplies it.
export default function AnnouncementsScreen({ embedded = false }: { embedded?: boolean }) {
  ...
  const inner = (
    <>
      ...
      {embedded ? null : <SectionTitle>Announcements</SectionTitle>}
      {items === null ? (
        <Loading />
      ) : items.length ? (
        items.map((b, i) => (
  ...
  if (embedded) {
    return (
      <View>
        {inner}
        {flash ? <View style={styles.toastInline}><Text style={styles.toastTxt}>{flash}</Text></View> : null}
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.body}>{inner}</ScrollView>
...
const styles = StyleSheet.create({
  ...
  toastInline: { alignItems: 'center', paddingTop: theme.sp.sm },
  toastTxt: {
